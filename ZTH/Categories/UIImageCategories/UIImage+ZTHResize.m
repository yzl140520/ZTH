...
#import "UIImage+ZTHResize.h"

@implementation UIImage (ZTHResize)
...
{
    return [self zth_resizableImageWithName:name leftFactor:.5f topFactor:.5f];
}
...
{
    return [self zth_blendImage:image withSize:size withCoverImage:coverImage withCoverImageSize:size];
}
...
{
    return [self zth_blendImage:image withSize:size withCoverImage:coverImage withCoverImageSize:coverImageSize scale:[UIScreen mainScreen].scale];
}
...
- (UIImage *)zth_resizedImage:(CGSize)newSize interpolationQuality:(CGInterpolationQuality)quality {
    ...
    return [self zth_resizedImage:newSize
...
- (UIImage *)zth_resizedImageWithContentMode:(UIViewContentMode)contentMode
    ...
    return [self zth_resizedImage:newSize interpolationQuality:quality];
}

// Returns a copy of the image that has been transformed using the given affine transform and scaled to the new size
// The new image's orientation will be UIImageOrientationUp, regardless of the current image's orientation
// If the new size is not integral, it will be rounded up
- (UIImage *)zth_resizedImage:(CGSize)newSize
